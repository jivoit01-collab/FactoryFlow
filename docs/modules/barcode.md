...
- **Split** (`PalletSplitPage`) `useSplitPallet`, **Box Transfer** (`BoxTransferPage`) `useTransferBoxes`, **Dismantle** (`DismantlePage`) `useDismantlePallet`/`useDismantleBox`, **Repack** (`RepackPage`) `useRepack`. Loose stock created by dismantle is **pooled per item code** on `/barcode/loose` (`useLooseStockSummary`; expand a row for the per-dismantle records, "History" toggle for the flat ledger). Repack takes an item + any qty (≤ pool total) + destination warehouse + batch (suggested when the pool has one batch), and the backend consumes the pool FIFO — the page previews which records will be drawn. Ticking rows in the source-records table optionally restricts consumption to those dismantled boxes (`loose_ids`), with qty then validated against the selection.
