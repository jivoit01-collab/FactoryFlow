...
- **Backend `wms` app** — the sole persistence path (`storage/apiAdapter.ts`, BASE `/wms`, i.e. `/api/v1/wms/...`). Company scoping rides on the `Company-Code` header that `core/api/client.ts` attaches from the selected company. One read-only exception: `adapter.list(collection, { allCompanies: true })` sends `?all_companies=1`, which the backend answers from every company the user belongs to — for a screen that measures the physical site rather than one company's books (the Warehouse Control board's pallet-space panel). It must never be fed into `wmsStore`, whose cache the per-company screens read *and write* through; the board holds its own react-query copy.
