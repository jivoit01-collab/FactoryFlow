...
export default function VisitorInsidePage() {
  ...
  // Filter to visitors only, then apply search
  const visitors = useMemo(() => {
    const visitorList = insideList.filter((entry) => entry.visitor);
    if (!search.trim()) return visitorList;
      ...
      <div className="flex items-center gap-3">
        ...
        {exitingId === entry.id ? (
          <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
            ...
            <Button size="sm" variant="ghost" onClick={() => { setExitingId(null); setSelectedGate(null); }}>
              Cancel
            </Button>
          </div>
        ) : (
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-3xl font-bold tracking-tight">Visitors Inside</h2>
            <p className="text-muted-foreground">
              {visitors.length} {visitors.length === 1 ? 'visitor' : 'visitors'} currently inside
            </p>
          </div>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-64 text-muted-foreground rounded-md border">
          ...
          <p className="text-lg">No visitors inside currently</p>
        </div>
      ) : (
        <Card>
