...
import { GateSelect, VisitorSelect } from '../../components/personGateIn';
...
export default function NewVisitorEntryPage() {
  ...
  const handleVisitorChange = (visitor: Visitor | null) => {
    ...
    setFormData((prev) => ({ ...prev, visitor: visitor?.id || null }));
    if (apiErrors.visitor) {
      setApiErrors((prev) => { const n = { ...prev }; delete n.visitor; return n; });
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.visitor) errors.visitor = 'Please select a visitor';
    ...
    try {
      ...
      navigate(`/gate/visitor/entry/${entry.id}`);
    } catch (error: unknown) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-3xl font-bold tracking-tight">New Visitor Entry</h2>
          <p className="text-muted-foreground">Create a new gate entry for a visitor</p>
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="space-y-4">
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base">Select Visitor</CardTitle>
            </CardHeader>
              ...
              {selectedVisitor && (
                <div className="p-3 rounded-lg border-2 border-primary bg-primary/5">
                  ...
                  {selectedVisitor.company_name && (
                    <p className="text-sm text-muted-foreground">{selectedVisitor.company_name}</p>
                  )}
                </div>
              )}
            </CardContent>
        ...
        <div className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <Input value={formData.purpose} onChange={(e) => setFormData((prev) => ({ ...prev, purpose: e.target.value }))} placeholder="e.g., Meeting with HR" className="mt-1" />
              </div>
              <div>
          ...
          <div className="flex gap-3">
            <Button variant="outline" onClick={() => navigate('/gate/visitor')} className="flex-1">Cancel</Button>
