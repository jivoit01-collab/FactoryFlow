...
export default function LabourInsidePage() {
  ...
  const labours = useMemo(() => {
    const labourList = insideList.filter((entry) => entry.labour);
    if (!search.trim()) return labourList;
    const searchLower = search.toLowerCase();
    return labourList.filter(
      (entry) =>
  ...
  const handleExit = async (entryId: number) => {
    ...
    try {
      await exitMutation.mutateAsync({ id: entryId, data: { gate_out: selectedGate } });
      setExitingId(null);
      ...
      <div className="flex items-center gap-3">
        ...
        {exitingId === entry.id ? (
          <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
            <div className="w-40">
              <GateSelect value={selectedGate ? String(selectedGate) : undefined} onChange={(gateId) => setSelectedGate(gateId)} placeholder="Select Gate" />
            </div>
            <Button size="sm" variant="default" onClick={() => handleExit(entry.id)} disabled={!selectedGate || exitMutation.isPending}>
              {exitMutation.isPending ? 'Exiting...' : 'Confirm'}
            </Button>
            <Button size="sm" variant="ghost" onClick={() => { setExitingId(null); setSelectedGate(null); }}>Cancel</Button>
          </div>
        ) : (
          <Button size="sm" variant="outline" onClick={(e) => { e.stopPropagation(); setExitingId(entry.id); }}>
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-3xl font-bold tracking-tight">Labours Inside</h2>
            <p className="text-muted-foreground">
              {labours.length} {labours.length === 1 ? 'labour' : 'labours'} currently inside
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="relative flex-1 max-w-sm">
        ...
        <Input placeholder="Search by name, purpose, gate..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10" />
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-64 text-muted-foreground rounded-md border">
          ...
          <p className="text-lg">No labours inside currently</p>
        </div>
      ) : (
        <Card>
