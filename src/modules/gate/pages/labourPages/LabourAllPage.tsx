...
export default function LabourAllPage() {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-3xl font-bold tracking-tight">All Labour Entries</h2>
            <p className="text-muted-foreground">
              View all labour gate entries
              {statusFilter && (
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
        ...
        <div className="w-full sm:w-auto">
          <DateRangePicker
            ...
            onDateChange={(date) => {
              if (date && 'from' in date) setDateRange(date);
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-64 text-muted-foreground rounded-md border">
          <p className="text-lg">No labour entries found</p>
        </div>
      ) : (
              ...
              <tbody>
                {filteredData.map((entry) => (
                  ...
                  >
                    ...
                    <td className="p-3 text-sm text-muted-foreground">{formatDateTime(entry.entry_time)}</td>
                    <td className="p-3 text-sm text-muted-foreground">{formatDateTime(entry.exit_time)}</td>
                    ...
                    <td className="p-3 text-sm text-muted-foreground max-w-[200px] truncate">{entry.purpose || '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
