...
import { GateInStatusBadge } from './GateInStatusBadge'
import type { GateInEntry } from '../types/gateIn.types'
...
export function GateInTable({ data, onDelete, isLoading }: GateInTableProps) {
        ...
        <tbody>
          {data.map((entry) => (
            <tr key={entry.id} className="border-t">
              ...
              <td className="p-3 text-sm">{formatDateTime(entry.entryTime)}</td>
              <td className="p-3">
                <GateInStatusBadge status={entry.status} />
              </td>
              <td className="p-3">
                <div className="flex items-center gap-2">
                  <Button
                    variant="ghost"
                    size="icon"
                    onClick={() => navigate(`/gate-in/${entry.id}`)}
                  >
                  ...
                  <Button
                    variant="ghost"
                    size="icon"
                    onClick={() => navigate(`/gate-in/${entry.id}?edit=true`)}
                  >
