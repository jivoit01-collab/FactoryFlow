import { FileText, Loader2, Paintbrush, Upload } from 'lucide-react';
  ...
  async function handleSave() {
    try {
      if (isRevision && record) {
        ...
        toast.success(`${record.item_code} revised to ${documentNumber.trim()}`);
      } else {
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            {isRevision
              ...
              : 'Record the document number, the barcode and both files for one label or carton.'}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          {isRevision ? (
            <div className="rounded-md border bg-muted/40 p-3">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Currently {record?.document_number} rev {record?.revision_label} of{' '}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                Leave blank for a carton that carries none. SAP holds no barcode for these
                items, so this is the only place it is recorded.
              </p>
            </div>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1">
              ...
              />
              <p className={`text-xs ${oversizePdf ? 'text-destructive' : 'text-muted-foreground'}`}>
                {oversizePdf
            ...
            <div className="space-y-1">
              ...
              />
              <p className={`text-xs ${oversizeCdr ? 'text-destructive' : 'text-muted-foreground'}`}>
                {oversizeCdr
