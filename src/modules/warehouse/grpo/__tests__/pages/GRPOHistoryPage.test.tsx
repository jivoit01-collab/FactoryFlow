...
function readSource(): string {
  ...
  return readFileSync(
    resolve(process.cwd(), 'src/modules/warehouse/grpo/pages/GRPOHistoryPage.tsx'),
    'utf-8',
  );
}
...
describe('GRPOHistoryPage — Header', () => {
  ...
  it('has back button navigating to /grpo', () => {
    const content = readSource();
    expect(content).toContain("navigate('/warehouse/grpo/material')");
  });
});
...
describe('GRPOHistoryPage — History List', () => {
  ...
  it('navigates to detail page on click', () => {
    const content = readSource();
    expect(content).toContain('navigate(`/warehouse/grpo/material/history/${entry.id}`)');
  });
