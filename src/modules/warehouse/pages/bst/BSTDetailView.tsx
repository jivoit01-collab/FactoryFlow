import { ExternalLink, Loader2, PackageCheck, Printer, ScanLine, XCircle } from 'lucide-react';
...
/**
 * Everything a branch stock transfer shows, with no opinion about where it is
 * shown. `BSTDetailPage` renders it under the usual dashboard header; the SAP
 * report grid renders the same thing inside a modal, so the BST behind a stock
 * movement can be read without losing the report.
 *
 * The print is identical in both: react-to-print copies the off-screen host
 * into its own iframe, so being inside a modal makes no difference to it.
 */
export interface BSTDetailViewProps {
  transferId: number;
  /**
   * 'page' puts the title and actions in a DashboardHeader. 'dialog' leaves the
   * title to the dialog and renders just the action row, plus a way out to the
   * full page.
   */
  mode?: 'page' | 'dialog';
  /**
   * Drops the actions that change the transfer (resume scanning, cancel),
   * leaving the record and its print. Used where the BST is being read as
   * evidence -- drilled into from a report row -- rather than worked on.
   */
  readOnly?: boolean;
}

export function BSTDetailView({ transferId, mode = 'page', readOnly = false }: BSTDetailViewProps) {
  ...
  const canResume = !readOnly && (t.status === 'SCANNING' || t.status === 'DRAFT' || liveActive);
  const canCancel =
    !readOnly && !['RECEIVED', 'PARTIALLY_RECEIVED', 'CLOSED', 'CANCELLED'].includes(t.status);
  ...
  const actions = (
    <div className="flex flex-wrap items-center gap-2">
      ...
      {/* In a modal the full page is still one click away — for a bookmark, a
          second tab, or anything the dialog crops. */}
      {mode === 'dialog' && (
        <Button variant="outline" onClick={() => navigate(`/warehouse/bst/${transferId}`)}>
          <ExternalLink className="h-4 w-4 mr-1" /> Open full page
        </Button>
      )}
      {canResume && (
