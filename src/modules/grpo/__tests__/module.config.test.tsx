...
function readSource(): string {
  ...
  return readFileSync(
    resolve(process.cwd(), 'src/modules/grpo/module.config.tsx'),
    'utf-8',
  )
}
...
describe('GRPO Module Config — Lazy Pages', () => {
  it('lazy loads GRPODashboardPage', () => {
    const content = readSource()
    expect(content).toContain("const GRPODashboardPage = lazy(() => import('./pages/GRPODashboardPage'))")
  })

  it('lazy loads PendingEntriesPage', () => {
    const content = readSource()
    expect(content).toContain("const PendingEntriesPage = lazy(() => import('./pages/PendingEntriesPage'))")
  })

  it('lazy loads GRPOPreviewPage', () => {
    const content = readSource()
    expect(content).toContain("const GRPOPreviewPage = lazy(() => import('./pages/GRPOPreviewPage'))")
  })

  it('lazy loads GRPOHistoryPage', () => {
    const content = readSource()
    expect(content).toContain("const GRPOHistoryPage = lazy(() => import('./pages/GRPOHistoryPage'))")
  })

  it('lazy loads GRPOHistoryDetailPage', () => {
    const content = readSource()
    expect(content).toContain("const GRPOHistoryDetailPage = lazy(() => import('./pages/GRPOHistoryDetailPage'))")
  })
})
