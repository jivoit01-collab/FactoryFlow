...
function makeCompany(overrides: Partial<UserCompany> = {}): UserCompany {
  return {
    company_id: 1, company_name: 'Corp', company_code: 'C',
    role: 'admin', is_default: true, is_active: true, ...overrides,
  }
}
...
describe('Auth Sync Middleware', () => {
  ...
  it('calls updateTokens on updateTokens action', () => {
    const action = updateTokens({
      access: 'new-a', refresh: 'new-r', expiresIn: 999, refreshExpiresAt: 5000,
    })
    middleware(action)
  ...
  it('calls updateUser on updateUser action', () => {
    const user: User = {
      id: 1, email: 'a@b.com', full_name: 'A', employee_code: 'E',
      is_active: true, is_staff: false, date_joined: '2024-01-01',
