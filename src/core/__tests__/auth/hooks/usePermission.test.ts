...
const mockAuthState = {
  permissions: ['gatein.view_gateinentry', 'gatein.add_gateinentry', 'qualitycheck.view_inspection'],
  user: {
    is_staff: false,
    companies: [
...
import { usePermission, useHasPermission, useCanPerformAction } from '@/core/auth/hooks/usePermission'
...
describe('usePermission', () => {
  ...
  it('hasAllPermissions returns true when all match', () => {
    const { result } = renderHook(() => usePermission())
    expect(result.current.hasAllPermissions(['gatein.view_gateinentry', 'gatein.add_gateinentry'])).toBe(true)
  })

  it('hasAllPermissions returns false when one is missing', () => {
    const { result } = renderHook(() => usePermission())
    expect(result.current.hasAllPermissions(['gatein.view_gateinentry', 'unknown.perm'])).toBe(false)
  })
