...
function readSource(): string {
  ...
  return readFileSync(
    resolve(process.cwd(), 'src/core/auth/utils/tokenRefresh.util.ts'),
    'utf-8',
  )
}
...
describe('auth/utils/tokenRefresh.util.ts', () => {
  ...
  it('exports refreshAccessToken as async function returning TokenRefreshResult', () => {
    const content = readSource()
    expect(content).toMatch(/export async function refreshAccessToken\(\): Promise<TokenRefreshResult>/)
  })
