...
function readSource(): string {
  ...
  return readFileSync(resolve(process.cwd(), 'src/core/store/rootReducer.ts'), 'utf-8');
}
...
describe('store/rootReducer.ts — Imports', () => {
  ...
  it('imports authReducer as default from @/core/auth/store/authSlice', () => {
    ...
    expect(content).toMatch(
      /import\s+authReducer\s+from\s+['"]@\/core\/auth\/store\/authSlice['"]/,
    );
  });
  ...
  it('imports notificationReducer as default from ./slices/notification.slice', () => {
    const content = readSource();
    expect(content).toMatch(
      /import\s+notificationReducer\s+from\s+['"]\.\/slices\/notification\.slice['"]/,
    );
  });
});
