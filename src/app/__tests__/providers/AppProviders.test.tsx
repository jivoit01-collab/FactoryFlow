...
function readSource(): string {
  ...
  return readFileSync(resolve(process.cwd(), 'src/app/providers/AppProviders.tsx'), 'utf-8');
}
...
describe('AppProviders', () => {
  ...
  it('imports Provider from react-redux', () => {
    const content = readSource();
    expect(content).toMatch(/import\s*\{[^}]*Provider[^}]*\}\s*from\s*['"]react-redux['"]/);
  });
  ...
  it('imports store from @/core/store and queryClient from @/core/api', () => {
    ...
    expect(content).toMatch(/import\s*\{[^}]*queryClient[^}]*\}\s*from\s*['"]@\/core\/api['"]/);
  });
