...
function readSource(): string {
  ...
  return readFileSync(resolve(process.cwd(), 'src/app/routes/AppRoutes.tsx'), 'utf-8');
}
...
describe('AppRoutes', () => {
  ...
  it('imports Suspense from react', () => {
    const content = readSource();
    expect(content).toMatch(/import\s*\{[^}]*Suspense[^}]*\}\s*from\s*['"]react['"]/);
  });
  ...
  it('imports ProtectedRoute from @/core/auth', () => {
    const content = readSource();
    expect(content).toMatch(/import\s*\{[^}]*ProtectedRoute[^}]*\}\s*from\s*['"]@\/core\/auth['"]/);
  });
