...
function readSource(): string {
  ...
  return readFileSync(resolve(process.cwd(), 'src/app/App.tsx'), 'utf-8');
}
...
describe('App', () => {
  ...
  it('imports AppProviders from ./providers', () => {
    const content = readSource();
    expect(content).toMatch(/import\s*\{[^}]*AppProviders[^}]*\}\s*from\s*['"]\.\/providers['"]/);
  });

  it('imports AppRoutes from ./routes', () => {
    const content = readSource();
    expect(content).toMatch(/import\s*\{[^}]*AppRoutes[^}]*\}\s*from\s*['"]\.\/routes['"]/);
  });
