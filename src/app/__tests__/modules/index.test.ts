...
describe('Module Registry', () => {
  ...
  it('imports type Reducer from @reduxjs/toolkit', () => {
    ...
    expect(content).toMatch(
      /import\s+type\s*\{[^}]*Reducer[^}]*\}\s*from\s*['"]@reduxjs\/toolkit['"]/,
    );
  });
  ...
  it('exports getAllNavigation function returning ModuleNavItem[]', () => {
    const content = readSource();
    expect(content).toMatch(/export\s+function\s+getAllNavigation\(\):\s*ModuleNavItem\[\]/);
  });
  ...
  it('exports getAllReducers function returning Record<string, Reducer>', () => {
    const content = readSource();
    expect(content).toMatch(/export\s+function\s+getAllReducers\(\):\s*Record<string,\s*Reducer>/);
  });
