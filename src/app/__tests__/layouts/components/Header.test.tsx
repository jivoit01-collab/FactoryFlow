...
function readSource(): string {
  ...
  return readFileSync(resolve(process.cwd(), 'src/app/layouts/components/Header.tsx'), 'utf-8');
}
...
describe('Header', () => {
  ...
  it('imports Menu, Moon, Sun, LogOut, User, Monitor from lucide-react', () => {
    const content = readSource();
    const iconImport = content.match(/import\s*\{([^}]*)\}\s*from\s*['"]lucide-react['"]/);
  ...
  it('imports useNavigate from react-router-dom', () => {
    const content = readSource();
    expect(content).toMatch(/import\s*\{[^}]*useNavigate[^}]*\}\s*from\s*['"]react-router-dom['"]/);
  });

  it('imports useAuth from @/core/auth', () => {
    const content = readSource();
    expect(content).toMatch(/import\s*\{[^}]*useAuth[^}]*\}\s*from\s*['"]@\/core\/auth['"]/);
  });

  it('imports useTheme from @/shared/contexts', () => {
    const content = readSource();
    expect(content).toMatch(/import\s*\{[^}]*useTheme[^}]*\}\s*from\s*['"]@\/shared\/contexts['"]/);
  });
