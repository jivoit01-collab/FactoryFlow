...
6. **A7 / A6 — dispatch & gate aggregation (decided).** **A7a FIXED** — Dispatch module + Service-GRPO submenu re-gated on dispatch-owned `can_post_bilty_service_grpo` (material-GRPO clerks no longer see Dispatch; `shahrukh@jivo.in` affected — grant him `can_post_bilty_service_grpo` if he does service-GRPO). **A7b (Docking) and A6 (returnable-gate) kept as intended aggregation** per decision.
